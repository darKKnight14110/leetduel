...
# editing either Dockerfile; image tags are fixed (":dev"), so a rebuild
# simply replaces what's referenced next time a container is created.

$ErrorActionPreference = "Stop"
$root = Split-Path -Parent $PSScriptRoot

docker build -t leetduel-sandbox-python:dev "$root/docker/sandbox-python"
docker build -t leetduel-sandbox-java:dev "$root/docker/sandbox-java"

Write-Host "Built leetduel-sandbox-python:dev and leetduel-sandbox-java:dev"
